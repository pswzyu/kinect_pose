...
double calculateAngle(nite::Point3f a, nite::Point3f b)
{
...
void SampleViewer::processPose(nite::UserTracker* pUserTracker, const nite::UserData& userData)
{
    nite::Point3f j_head = userData.getSkeleton().getJoint(nite::JOINT_HEAD).getPosition();
    nite::Point3f j_neck = userData.getSkeleton().getJoint(nite::JOINT_NECK).getPosition();
    nite::Point3f j_lshoulder = userData.getSkeleton().getJoint(nite::JOINT_LEFT_SHOULDER).getPosition();
    nite::Point3f j_lelbow = userData.getSkeleton().getJoint(nite::JOINT_LEFT_ELBOW).getPosition();
    nite::Point3f j_lhand = userData.getSkeleton().getJoint(nite::JOINT_LEFT_HAND).getPosition();
    nite::Point3f j_rshoulder = userData.getSkeleton().getJoint(nite::JOINT_RIGHT_SHOULDER).getPosition();
    nite::Point3f j_relbow = userData.getSkeleton().getJoint(nite::JOINT_RIGHT_ELBOW).getPosition();
    nite::Point3f j_rhand = userData.getSkeleton().getJoint(nite::JOINT_RIGHT_HAND).getPosition();
    nite::Point3f j_torso = userData.getSkeleton().getJoint(nite::JOINT_TORSO).getPosition();
    nite::Point3f j_lhip = userData.getSkeleton().getJoint(nite::JOINT_LEFT_HIP).getPosition();
    nite::Point3f j_lknee = userData.getSkeleton().getJoint(nite::JOINT_LEFT_KNEE).getPosition();
    nite::Point3f j_lfoot = userData.getSkeleton().getJoint(nite::JOINT_LEFT_FOOT).getPosition();
    ...
    {
        pose_stream.pop_front();
    }

    mat likelihood = predictor.Predict(pose_stream);
    uword row_max_index;
    uword prediction;
	...
	for (int i = 0; i < users.getSize(); ++i)
	{
		...
		else if (!user.isLost())
		{
			...
			if (users[i].getSkeleton().getState() == nite::SKELETON_TRACKED && g_drawSkeleton)
			{
                ...
                processPose(m_pUserTracker, user);
			}
		}
